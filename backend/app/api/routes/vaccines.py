...
from app.models.all_models import User, DogProfile, VaccineRecord
from app.schemas.schemas import VaccineCreate, VaccineOut
...
def get_vaccines(dog_id: UUID, clerk_user_id: str = Depends(get_current_user), db: Session = Depends(get_db)):
    _verify_dog_ownership(dog_id, clerk_user_id, db)
    return db.query(VaccineRecord).filter(VaccineRecord.dog_id == dog_id).all()

@router.post("/{dog_id}/vaccines", response_model=VaccineOut, status_code=201)
def create_vaccine(dog_id: UUID, payload: VaccineCreate, clerk_user_id: str = Depends(get_current_user), db: Session = Depends(get_db)):
    _verify_dog_ownership(dog_id, clerk_user_id, db)
